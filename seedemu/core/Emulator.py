...
class Emulator(Loggable):
    ...
    def __render(self, layerName, optional: bool, configure: bool):
        ...
        self._log('requesting {}: {}'.format(verb, layerName))

        if optional and layerName not in self.__layers.db:
            self._log('{}: not found but is optional, skipping'.format(layerName))
            return
        ...
        if done:
            self._log('{}: already done, skipping'.format(layerName))
            return

        if layerName in self.__dependencies_db:
            for (dep, opt) in self.__dependencies_db[layerName]:
                self._log('{}: requesting dependency render: {}'.format(layerName, dep))
                self.__render(dep, opt, configure)

        self._log('entering {}...'.format(layerName))
        ...
        if configure:
            self._log('invoking pre-configure hooks for {}...'.format(layerName))
            for hook in hooks: hook.preconfigure(self)
            self._log('configureing {}...'.format(layerName))
            layer.configure(self)
            self._log('invoking post-configure hooks for {}...'.format(layerName))
            for hook in hooks: hook.postconfigure(self)
        else:
            self._log('invoking pre-render hooks for {}...'.format(layerName))
            for hook in hooks: hook.prerender(self)
            self._log('rendering {}...'.format(layerName))
            layer.render(self)
            self._log('invoking post-render hooks for {}...'.format(layerName))
            for hook in hooks: hook.postrender(self)
        
        self._log('done: {}'.format(layerName))
    ...
    def _log(self, message: str):
        ...
        """
        super().__log__("Emulator", message)
    ...
    def render(self) -> Emulator:
        ...
        self._log('collecting virtual node names in the emulation...')
        vnodes: List[str] = []
        for (layer, _) in self.__layers.db.values():
            ...
            for (vnode, _) in layer.getPendingTargets().items():
                ...
                vnodes.append(vnode)
        self._log('found {} virtual nodes.'.format(len(vnodes)))

        # resolv bindings for all vnodes
        self._log('resolving binding for all virtual nodes...')
        for binding in self.getBindings():
            for vnode in vnodes:
                if vnode in self.__resolved_bindings: continue
                pnode = binding.getCandidate(vnode, self)
                if pnode == None: continue
                self._log('vnode {} bound to as{}/{}'.format(vnode, pnode.getAsn(), pnode.getName()))
                self.__resolved_bindings[vnode] = pnode

        self._log('applying changes made to virtual physical nodes to real physical nodes...')
        ...
        for (vnode, pnode) in self.__resolved_bindings.items():
            ...
            self._log('applying changes made on vnode {} to pnode as{}/{}...'.format(vnode, pnode.getAsn(), pnode.getName()))
