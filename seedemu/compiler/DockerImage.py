...
class DockerImage(object):
    ...
    def getSoftware(self) -> Set[NodeSoftware]:
        ...
        """
        if self.__baseImage:
            return self.__software.union(self.__baseImage.getSoftware())
        return self.__software
    
    def addFile(self, file: NodeFile) -> DockerImage:
        """!
        @brief add a file to the docker image. It will overwrite file at path if it already exists.
        @param file the file to add to the image

        @returns self, for chaining api calls.
        """
        self.__files[file.getPath()] = file
        return self
    ...
    def setLocal(self, isLocal: bool) -> DockerImage:
        """!
        @brief sets the image as a local image
        @param isLocal true to set the image to local, false to set the image to remote
    ...
    def generateImageSetup(self, useRunCommand=True):
        """!
        ...
        if len(self.__files) > 0:
            ...
            for file in self.__files.values():
                dockerfile += self.__addFile(file)

        # Try to reduce image size a bit.
        dockerfile += "\n# Reduce image size a bit\n"
        dockerfile += "RUN apt-get clean\n"
        dockerfile += "RUN rm -r /var/lib/apt/lists/*\n"
    ...
    def __runFile(cls, file: NodeFile) -> str:
        """!
        @brief Add the file to docker & execute it, return related commands to dockerfile string.
        ...
        """
        dockerfile = cls.__addFile(file)
        dockerfile += f'RUN .{file.getPath()}\n'
        return dockerfile

    def __addFile(cls, file: NodeFile) -> str:
        """!
