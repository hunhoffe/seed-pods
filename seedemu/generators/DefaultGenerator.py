...
class DefaultGenerator:
    ...
    def __generate(self, asn: int, emulator: Emulator, depth: int):
        ...
        self.logger.info('generating AS{}...'.format(asn))
        ...
        if asn in base.getAsns():
            self.logger.info('AS{} already done, skipping...'.format(asn))
            return
        
        self.logger.info('getting list of IXes joined by AS{}...'.format(asn))
        ixes = self.__provider.getInternetExchanges()

        self.logger.info('getting list of prefixes announced by AS{}...'.format(asn))
        prefixes = self.__provider.getPrefixes()

        self.logger.info('getting list of peers of AS{}...'.format(asn))
        ...
        self.logger.info('creating {} networks for AS{}...'.format(len(prefixes), asn))
        for prefix in prefixes:
            netname = 'net{}'.format(net_count)

            self.logger.info('creating {} with prefix {} for AS{}...'.format(netname, prefix, asn))
        ...
        self.logger.info('looking for details of {} IXes joined by AS{}...'.format(len(ixes), asn))
        for ix in ixes:
            if ix in base.getInternetExchangeIds():
                self.logger.info('IX{} already created, skipping...')
                continue

            self.logger.info('creating new IX, IX{}; getting prefix...'.format(ix))
            base.createInternetExchange(ix, prefix = self.__provider.getInternetExchangePrefix(ix))

            self.logger.info('getting members of IX{}...'.format(ix))
            members = self.__provider.getInternetExchangeMembers(ix)

            self.logger.info('joining IX{} with AS{}...'.format(ix, asn))
            router.joinNetwork('ix{}'.format(ix), members[asn])
            
            self.logger.info('creating {} other ASes in IX{}...'.format(len(members.keys()), ix))
            for member in members.keys():
                self.__generate(member, emulator, depth - 1)
                if member in peers.keys():
                    # FIXME: right = peer is customer, left = peer is provider
                    rel = peers[member]
                    self.logger.info('peering AS{} with AS{} in IX{} using relationship {}...'.format(member, asn, ix, rel))
