...
from . .core import Emulator, AutonomousSystem, InternetExchange, Loggable
from . .layers import Base, Routing, Ebgp, Ibgp, Ospf

class DefaultGenerator(Loggable):
    """!
    ...
    def _log(self, message: str) -> None:
        ...
        """
        super.__log__("DefaultGenerator", message)
    ...
    def __generate(self, asn: int, emulator: Emulator, depth: int):
        ...
        self._log('generating AS{}...'.format(asn))
        ...
        if asn in base.getAsns():
            self._log('AS{} already done, skipping...'.format(asn))
            return
        
        self._log('getting list of IXes joined by AS{}...'.format(asn))
        ixes = self.__provider.getInternetExchanges()

        self._log('getting list of prefixes announced by AS{}...'.format(asn))
        prefixes = self.__provider.getPrefixes()

        self._log('getting list of peers of AS{}...'.format(asn))
        ...
        self._log('creating {} networks for AS{}...'.format(len(prefixes), asn))
        for prefix in prefixes:
            netname = 'net{}'.format(net_count)

            self._log('creating {} with prefix {} for AS{}...'.format(netname, prefix, asn))
        ...
        self._log('looking for details of {} IXes joined by AS{}...'.format(len(ixes), asn))
        for ix in ixes:
            if ix in base.getInternetExchangeIds():
                self._log('IX{} already created, skipping...')
                continue

            self._log('creating new IX, IX{}; getting prefix...'.format(ix))
            base.createInternetExchange(ix, prefix = self.__provider.getInternetExchangePrefix(ix))

            self._log('getting members of IX{}...'.format(ix))
            members = self.__provider.getInternetExchangeMembers(ix)

            self._log('joining IX{} with AS{}...'.format(ix, asn))
            router.joinNetwork('ix{}'.format(ix), members[asn])
            
            self._log('creating {} other ASes in IX{}...'.format(len(members.keys()), ix))
            for member in members.keys():
                self.__generate(member, emulator, depth - 1)
                if member in peers.keys():
                    # FIXME: right = peer is customer, left = peer is provider
                    rel = peers[member]
                    self._log('peering AS{} with AS{} in IX{} using relationship {}...'.format(member, asn, ix, rel))
